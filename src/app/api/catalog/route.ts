...
export async function GET() {
  try {
    ...
    const current =
      bootstrap.events.find((event: any) => event.is_current) ||
      bootstrap.events.find((event: any) => event.is_next) ||
      bootstrap.events.find((event: any) => !event.finished) ||
      bootstrap.events.at(-1);
    const finishedEvents = bootstrap.events
      .filter((event: any) => event.finished && event.data_checked)
    ...
    const players = bootstrap.elements.map((player: any) => ({
      ...
      teamId: player.team,
      position: position.get(player.element_type) || "MID",
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "FPL catalog unavailable" },
      { status: 502 },
    );
  }
}
